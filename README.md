...
*Left: percentage points of excess return over the benchmark by test year on the 110-name point-in-time universe, top-10 daily replay with no costs and no gate ([report](docs/research/current/SP500_PIT_GICS_TOP10_MULTIYEAR_BASELINE_2026-06-23.md)); beside it, the roughly 700-name panel's return compounded across its four yearly test windows against the benchmark's, with no transaction costs, no rank gate, and no single year significant on its own ([report](docs/research/current/PIT_MASKED_PANEL_2022_2025_FULL_RUN_REPORT_2026-05-16.md)). Right: each graph specification's mean daily IC difference from the graph-zeroed control on the same 238 test days with its 95% block-bootstrap interval; every mean is negative and none is distinguishable from zero, BHY-adjusted p = 1.00 for all four ([report](docs/research/current/GRAPH_PAIRED_REANALYSIS_2026-09-02.md)). Every number is read from those reports' tables ([`docs/assets/readme_evidence.json`](docs/assets/readme_evidence.json)); `python scripts/gen_readme_figure.py` regenerates the figure.*
