...
**Branch:** `codex/rearchitecture-phase0-20260704` (22 commits across the Phase 0–3 workstreams)
...
git fetch origin
git log --oneline codex/rearchitecture-phase0-20260704..origin/main
...
```
If `origin/main` gained new commits beyond the recorded snapshot, re-run the
`merge-tree` check; only proceed conflict-free.
...
gh pr create --base main --title "Rearchitecture phase 0: retained workstreams" --body-file docs/handoffs/2026-07-06-rearchitecture-merge-plan.md
```
...
3. **Communicate remaining path moves** (commit to `AGENTS.md`/`docs/QUICK_REFERENCE.md` if not already): `tests/backtest_sp500.py` → engine `mci_gru/evaluation/backtest_engine.py` + CLI `scripts/backtest_sp500.py`; `tests/backtest_sp500_daily.py` → `scripts/backtest_sp500_daily.py`.
4. **Drop the 3 stashes** (Step 5 above) and optionally prune the many stale detached-HEAD worktrees (`git worktree prune` won't remove them; use `git worktree remove <path>` per worktree once their chats are done).

## Risks (ranked)
1. **Cross-platform golden tests**: fixtures generated on Windows, CI runs ubuntu. Goldens use JSON with 1e-12 tolerance and pandas-parsed CSVs, so line endings/float formatting should be inert — but PR CI is the first true cross-platform run of the golden suite from a clean clone. Mitigation: it runs *before* merge; a failure blocks at Step 2, nothing lands broken.
2. **Old-path notebook** (`performance_proof_missing_grid_colab.ipynb`): silently depends on the deprecated shim. No merge-day breakage; becomes a landmine at shim removal if follow-up 2 is skipped.
3. **None found for**: docs SOT check (non-recursive, allowlist correct), paper_trade nightly (frozen checkpoints, paths already updated), `docs/workflows/` (no stale path references).
