...
## Merge completed 2026-07-06

WS-N steps 3–4 landed on branch `codex/rearchitecture-phase0-20260704`:

- **Fork collapsed:** `scripts/backtest_sp500_daily.py` is a thin CLI wrapper (~200 lines)
  that defines the 20 shared daily flags, rejects the six engine-only flags via argparse,
  and delegates to `mci_gru.evaluation.backtest_engine.main(argv)`.
- **Engine canonical:** `mci_gru/evaluation/backtest_engine.py` remains the sole
  implementation body; behavioral decisions §1–§5 all favor the engine side.
- **Daily semantics preserved:** wrapper forces `holding_period=1` (engine default),
  omits MLflow flags (tracking off), and does not expose `--rebalance_style`.
- **Goldens:** all 10 `tests/test_backtest_engine_golden.py` cases pass without
  regenerating daily goldens; only permitted daily-visible drift is §5 stdout text.
- **Shim:** `tests/backtest_sp500_daily.py` re-exports via `scripts/backtest_sp500_daily`
  (which re-exports the engine namespace).
