...
Test files: 74
Test functions: 429 (parametrized cases collapsed)
<!-- test-inventory-sha256: 9e1624f130884f68b14e6d9b1e82ee3c8a56cf64eb9389f0c1d1090a858345de -->
...
| Test | Description | Markers | Last run | Time (s) |
|---|---|---|---|---|
| `test_backtest_engine_golden` |  | parametrize | PASSED | 28.57 |
| `test_backtest_engine_deterministic` |  | parametrize | PASSED | 56.62 |
...
| Test | Description | Markers | Last run | Time (s) |
|---|---|---|---|---|
| `test_dispatch_refuses_pit_universe_when_simulator_cannot_admit_it` | A simulator that cannot take the PIT universe must not be run with one. | parametrize | PASSED | 0.61 |
| `test_shipped_simulators_all_accept_the_pit_universe` | Every dispatchable path admits on the PIT basis, so production never refuses. |  | PASSED | 0.73 |
| `test_pit_inactive_name_is_never_held` | The top-scoring name is held only when the PIT universe admits it. | parametrize | PASSED | 1.10 |
| `test_internal_benchmark_uses_the_pit_filtered_universe` | Each booked benchmark return is the equal-weight mean over PIT-active rows only. | parametrize | PASSED | 0.74 |
| `test_pit_run_matches_a_universe_that_never_contained_the_name` | Public evaluate(): a PIT-restricted run equals a genuinely restricted universe. | parametrize | PASSED | 1.30 |
| `test_saved_artifacts_exclude_pit_inactive_holdings` | The saved holdings artifacts inherit the same PIT admission basis. |  | PASSED | 0.49 |
| `test_dispatch_matches_direct_simulator_call_without_pit` | With no PIT universe the unified dispatch reproduces the legacy call exactly. | parametrize | PASSED | 1.36 |
...
| `test_prepare_data_training_labels_never_consume_validation_closes` | End-to-end: no training label may read a close at or after val_start. | parametrize | PASSED | 0.69 |
...
| Test | Description | Markers | Last run | Time (s) |
|---|---|---|---|---|
| `test_compute_regime_monthly_features_outputs_expected_columns` |  |  | PASSED | 0.90 |
| `test_compute_regime_monthly_features_adds_subsequent_return_signals` |  |  | PASSED | 0.01 |
| `test_compute_regime_monthly_features_no_lookahead_exclusion_effect` |  |  | PASSED | 1.94 |
| `test_regime_subsequent_return_horizon_excludes_future_window` |  |  | PASSED | 0.81 |
| `test_add_regime_features_broadcasts_without_row_change` |  |  | PASSED | 2.55 |
...
| `test_retired_package_stays_out_of_packaging_and_imports` |  |  | PASSED | 1.03 |
...
| `test_run_pytest_places_managed_paths_before_double_dash` |  |  | PASSED | 0.53 |
...
| `test_run_pytest_preserves_safe_config_addopts_and_overrides_cache_dir` |  |  | PASSED | 0.49 |
...
| Test | Description | Markers | Last run | Time (s) |
|---|---|---|---|---|
| `test_positive_complete_60_date_fixture_can_claim_preliminary_but_not_confirmatory` |  |  | PASSED | 2.86 |
| `test_negative_60_date_fixture_reports_no_detectable_signal` |  |  | PASSED | 0.80 |
...
| Test | Description | Markers | Last run | Time (s) |
|---|---|---|---|---|
| `test_saved_prediction_selection_research_never_invokes_training` |  |  | PASSED | 2.18 |
| `test_research_evidence_uses_canonical_sessions_and_keeps_invalid_dates` |  |  | PASSED | 0.12 |
| `test_future_market_rows_do_not_change_prior_dated_evidence` |  |  | PASSED | 0.19 |
...
| `test_pit_entry_and_exit_boundaries_change_only_the_declared_daily_denominator` |  |  | PASSED | 0.24 |
