...
Test files: 81
Test functions: 474 (parametrized cases collapsed)
<!-- test-inventory-sha256: 5a2210330128f29fa93e8641ce6e35fac0724577363b1f4fb5fb0276a08c14d2 -->
...
| Test | Description | Markers | Last run | Time (s) |
|---|---|---|---|---|
| `test_backtest_engine_golden` |  | parametrize | PASSED | 27.76 |
| `test_backtest_engine_deterministic` |  | parametrize | PASSED | 54.84 |
...
| Test | Description | Markers | Last run | Time (s) |
|---|---|---|---|---|
| `test_plot_equity_curve_uses_agg_backend_when_saving` |  |  | PASSED | 0.40 |
...
| `test_missing_selected_data_config_is_a_hard_error` |  |  | PASSED | 0.26 |
...
| `test_shipped_simulators_all_accept_the_pit_universe` | Every dispatchable path admits on the PIT basis, so production never refuses. |  | PASSED | 0.89 |
...
| `test_pit_run_matches_a_universe_that_never_contained_the_name` | Public evaluate(): a PIT-restricted run equals a genuinely restricted universe. | parametrize | PASSED | 1.37 |
...
| `test_dispatch_matches_direct_simulator_call_without_pit` | With no PIT universe the unified dispatch reproduces the legacy call exactly. | parametrize | PASSED | 1.56 |
...
| `test_prepare_data_training_labels_never_consume_validation_closes` | End-to-end: no training label may read a close at or after val_start. | parametrize | PASSED | 0.79 |
...
| Test | Description | Markers | Last run | Time (s) |
|---|---|---|---|---|
| `test_compute_regime_monthly_features_outputs_expected_columns` |  |  | PASSED | 1.15 |
| `test_compute_regime_monthly_features_adds_subsequent_return_signals` |  |  | PASSED | 0.02 |
| `test_compute_regime_monthly_features_no_lookahead_exclusion_effect` |  |  | PASSED | 2.64 |
| `test_regime_subsequent_return_horizon_excludes_future_window` |  |  | PASSED | 1.15 |
| `test_add_regime_features_broadcasts_without_row_change` |  |  | PASSED | 2.97 |
...
| `test_transform_with_regime_df_produces_nonzero_regime_columns` | Regime columns must be non-constant when a real regime_df is passed to transform. |  | PASSED | 0.68 |
...
| `test_retired_package_stays_out_of_packaging_and_imports` |  |  | PASSED | 0.39 |
...
| `test_run_pytest_places_managed_paths_before_double_dash` |  |  | PASSED | 0.54 |
...
| `test_run_pytest_rejects_config_addopts_that_hide_managed_paths` |  | parametrize | PASSED | 0.92 |
...
| Test | Description | Markers | Last run | Time (s) |
|---|---|---|---|---|
| `test_positive_complete_60_date_fixture_can_claim_preliminary_but_not_confirmatory` |  |  | PASSED | 1.08 |
| `test_negative_60_date_fixture_reports_no_detectable_signal` |  |  | PASSED | 1.01 |
...
| Test | Description | Markers | Last run | Time (s) |
|---|---|---|---|---|
| `test_saved_prediction_selection_research_never_invokes_training` |  |  | PASSED | 2.30 |
