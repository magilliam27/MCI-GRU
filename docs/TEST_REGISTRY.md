...
<!-- test-inventory-sha256: a6e2697c56b0777af9b3dfdfb6376d63f1add07c9a7a02f62ed0b930112ade07 -->
...
| Test | Description | Markers | Last run | Time (s) |
|---|---|---|---|---|
| `test_backtest_engine_golden` |  | parametrize | PASSED | 40.47 |
| `test_backtest_engine_deterministic` |  | parametrize | PASSED | 81.52 |
...
| `test_run_emits_no_unreadable_file_warning_for_any_data_source` | A source the script cannot inspect must not produce a bogus warning. |  | PASSED | 0.46 |
...
| Test | Description | Markers | Last run | Time (s) |
|---|---|---|---|---|
| `test_rank_drop_gate_matches_paper_trade_initial_and_survivor_behavior` |  |  | PASSED | 0.01 |
...
| `test_lambdarank_ic_notebook_code_cells_parse` |  |  | PASSED | 0.01 |
...
| `test_config_hydra` | Test Hydra configuration. |  | PASSED | 0.03 |
...
| `test_shipped_simulators_all_accept_the_pit_universe` | Every dispatchable path admits on the PIT basis, so production never refuses. |  | PASSED | 1.17 |
...
| `test_saved_artifacts_exclude_pit_inactive_holdings` | The saved holdings artifacts inherit the same PIT admission basis. |  | PASSED | 0.66 |
| `test_dispatch_matches_direct_simulator_call_without_pit` | With no PIT universe the unified dispatch reproduces the legacy call exactly. | parametrize | PASSED | 2.11 |
...
| `test_prepare_data_training_labels_never_consume_validation_closes` | End-to-end: no training label may read a close at or after val_start. | parametrize | PASSED | 0.99 |
...
| `test_compute_regime_monthly_features_no_lookahead_exclusion_effect` |  |  | PASSED | 3.05 |
| `test_regime_subsequent_return_horizon_excludes_future_window` |  |  | PASSED | 1.31 |
| `test_add_regime_features_broadcasts_without_row_change` |  |  | PASSED | 3.97 |
...
| `test_transform_with_regime_df_produces_nonzero_regime_columns` | Regime columns must be non-constant when a real regime_df is passed to transform. |  | PASSED | 0.83 |
...
| `test_run_pytest_preserves_failure_exit_and_cleans_root` |  |  | PASSED | 0.78 |
| `test_run_pytest_places_managed_paths_before_double_dash` |  |  | PASSED | 0.80 |
| `test_run_pytest_can_keep_temp_for_explicit_diagnostics` |  |  | PASSED | 0.79 |
| `test_run_pytest_contains_active_cache_and_removes_it` |  |  | PASSED | 0.77 |
| `test_run_pytest_rejects_config_addopts_that_hide_managed_paths` |  | parametrize | PASSED | 1.33 |
| `test_run_pytest_preserves_safe_config_addopts_and_overrides_cache_dir` |  |  | PASSED | 0.70 |
...
| `test_negative_60_date_fixture_reports_no_detectable_signal` |  |  | PASSED | 1.38 |
...
| Test | Description | Markers | Last run | Time (s) |
|---|---|---|---|---|
| `test_saved_prediction_selection_research_never_invokes_training` |  |  | PASSED | 3.00 |
...
| `test_unmatured_tail_outcome_is_reported_and_never_zeroed` |  |  | PASSED | 0.13 |
...
| `test_expected_denominator_covers_every_active_member_with_exclusion_reason` |  |  | PASSED | 0.20 |
...
| `test_active_constituents_on_date_uses_validity_intervals` |  |  | PASSED | 0.02 |
