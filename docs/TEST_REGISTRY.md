...
Test files: 80
Test functions: 468 (parametrized cases collapsed)
<!-- test-inventory-sha256: e3430cfd4597cf809d5d0ae3e56ddc78260826684796f039e58231075bc1010f -->
...
| Test | Description | Markers | Last run | Time (s) |
|---|---|---|---|---|
| `test_backtest_engine_golden` |  | parametrize | PASSED | 28.41 |
| `test_backtest_engine_deterministic` |  | parametrize | PASSED | 55.81 |
...
| `test_run_emits_no_unreadable_file_warning_for_any_data_source` | A source the script cannot inspect must not produce a bogus warning. |  | PASSED | 0.40 |
| `test_data_file_warning_is_gated_on_the_csv_source` |  |  | PASSED | 0.27 |
...
| `TestGATBlockEdgeDim.test_forward_with_4d_edge_attr` |  |  | PASSED | 0.08 |
...
| `test_shipped_simulators_all_accept_the_pit_universe` | Every dispatchable path admits on the PIT basis, so production never refuses. |  | PASSED | 0.99 |
| `test_pit_inactive_name_is_never_held` | The top-scoring name is held only when the PIT universe admits it. | parametrize | PASSED | 1.62 |
...
| `test_pit_run_matches_a_universe_that_never_contained_the_name` | Public evaluate(): a PIT-restricted run equals a genuinely restricted universe. | parametrize | PASSED | 1.70 |
...
| `test_leaver_rejoiner_masks_turn_off_and_back_on` |  |  | PASSED | 0.02 |
...
| `test_generate_time_series_features_duplicate_dt_kdcode_last_wins` | When duplicate (dt, kdcode) rows exist, last row wins (legacy iterrows behaviour). |  | PASSED | 0.02 |
...
| `test_regime_input_contract_columns_present` |  |  | PASSED | 0.01 |
| `test_regime_csv_contract_is_deprecated_and_requires_full_seven_variable_surface` | Deprecated CSV override must not silently drop paper-guided variables. |  | PASSED | 0.05 |
...
| `test_retired_package_stays_out_of_packaging_and_imports` |  |  | PASSED | 0.58 |
...
| `test_run_pytest_cleans_unique_root_after_success` |  |  | PASSED | 0.53 |
...
| `test_negative_60_date_fixture_reports_no_detectable_signal` |  |  | PASSED | 1.50 |
...
| Test | Description | Markers | Last run | Time (s) |
|---|---|---|---|---|
| `test_saved_prediction_selection_research_never_invokes_training` |  |  | PASSED | 2.29 |
...
| `test_permutation_is_deterministic_and_assignment_is_outcome_independent` |  |  | PASSED | 0.17 |
