...
_CREDENTIAL_ASSIGNMENT = re.compile(
    rb"""(?im)["']?\b(?:api[_-]?key|password|passwd|secret|token|access_token|private_key)\b["']?\s*[:=]\s*["'][^"'\r\n]+["']"""
)
    ...
    *,
    ...
    record = {
        ...
        "code_identity": {
            ...
            "git_observations": {"commit": commit, "status": {**status, "value": None}},
...
def _contains_credentials(content: bytes) -> bool:
    return bool(
        _CREDENTIAL_ASSIGNMENT.search(content)
        or _CREDENTIAL_URI.search(content)
        or b"PRIVATE KEY-----" in content
    )
...
def _validate_config(content: bytes) -> None:
    ...
    if not isinstance(payload, dict) or _contains_credentials(content):
...
def _git_observation(root: Path, *args: str) -> dict[str, Any]:
    try:
    ...
    return {"status": "observed", "value": result.stdout.decode("utf-8", errors="replace").strip()}
