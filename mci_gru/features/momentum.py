...
def _estimate_dynamic_fast_weights_for_group(
    ...
    for state_code in _DYNAMIC_STATES:
        ...
        second_sums[state_code] = _window_history(
            forward_sq.where(state_mask, 0.0), lookback_periods
        )
    ...
    second = {
        state_code: second_sums[state_code]
        / counts[state_code].where(counts[state_code] > 0, np.nan)
        for state_code in _DYNAMIC_STATES
    }
    probabilities = {
...
def _compute_blend_weights(
    ...
    if blend_mode not in MOMENTUM_BLEND_MODES:
        raise ValueError(f"blend_mode must be one of {MOMENTUM_BLEND_MODES}, got {blend_mode!r}")
...
def add_momentum_buffered(
    ...
    def compute_buffered_signal(group: pd.DataFrame, col: str) -> pd.Series:
        ...
        for i, rank in enumerate(ranks):
            if rank < buffer_low or rank > buffer_high:
                signals[i] = 0.0
            else:
    ...
    df["trade_signal"] = np.where(
        df["momentum_blend"].abs() < 0.2, 0, np.sign(df["momentum_blend"])
    )
