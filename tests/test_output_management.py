...
def test_output_structure():
    ...
    try:
        ...
        assert os.path.exists(os.path.join(run_dir, 'averaged_predictions')), "Predictions dir not created"
...
def test_logging_setup():
    ...
    try:
        ...
        log_files = [f for f in os.listdir(temp_dir) if f.startswith('backtest_') and f.endswith('.log')]
        ...
        if os.path.exists(temp_dir):
            try:
                ...
                print(f"  [WARNING] Could not remove temp directory: {temp_dir} (files still locked)")
                pass
