...
def haircut_sharpe_ratio(sharpe_ratio, num_years, num_tests, method="bhy", rank=1, alpha=0.05):
    """
...
def calculate_forward_returns(df, label_t=5):
    ...
    print(
        f"  Added returns: forward_return_{label_t}d, open_to_open_return, daily_return, overnight_gap"
    )
    print("  Portfolio & Benchmark use 'open_to_open_return' (same entry/exit window)")
    print("  'daily_return' kept for reference (standard S&P 500 methodology)")
...
def simulate_trading_strategy(
    ...
    print("  Portfolio: open-to-open returns (entry at T+1 open, exit at T+2 open)")
    print("  Benchmark: open-to-open returns (equal-weighted, same window as portfolio)")
    ...
    if gate_enabled:
        print(
            f"    Min rank drop: {min_rank_drop} (held stock exits only if rank worsens by >= {min_rank_drop})"
        )
    ...
    for _i, pred_date in enumerate(pred_dates):
        ...
        # Gate enabled -> only sell currently held names that worsen by >= min_rank_drop.
            ...
            refill_candidates = [
                kd for kd in day_preds["kdcode"].tolist() if kd not in survivor_set
            ]
        ...
        realized_holdings = top_k_data[["kdcode", "open_to_open_return"]].dropna(
            subset=["open_to_open_return"]
        )
                ...
                daily_holdings_records.append(
                    {
                        ...
                        "kdcode": held_kdcode,
                        "rank": int(current_ranks.get(held_kdcode, np.nan))
                        if held_kdcode in current_ranks
                        else np.nan,
            ...
            trade_records.append(
                {
                    ...
                    "action": "BUY",
                    "rank": int(current_ranks.get(bought_kdcode, np.nan))
                    if bought_kdcode in current_ranks
                    else np.nan,
            ...
            trade_records.append(
                {
                    ...
                    "kdcode": sold_kdcode,
                    "action": "SELL",
                    "rank": int(current_ranks.get(sold_kdcode, np.nan))
                    if sold_kdcode in current_ranks
                    else np.nan,
        ...
        for _, row in day_preds.iterrows():
            ...
            actual = pred_date_data[pred_date_data["kdcode"] == kdcode][
    ...
    if gate_enabled:
        ...
        print(
            f"  Rank-drop gate: {days_skipped_by_rank_gate} days skipped (empty post-gate portfolio)"
        )
    ...
    return {
        ...
        "portfolio_returns": np.array(net_portfolio_returns)
        if tc_enabled
        else np.array(gross_portfolio_returns),
...
def simulate_trading_strategy_staggered(
    predictions_df,
    stock_data_df,
    top_k=10,
    label_t=5,
    ...
    for i, pred_date in enumerate(pred_dates):
        ...
        if not gate_enabled or not tranche["holdings"] or tranche["prev_ranks"] is None:
            top_stocks = day_preds.head(top_k)["kdcode"].tolist()
        else:
        ...
        for _tid, t in tranches.items():
            if t["holdings"]:
                t_returns = entry_day_data[entry_day_data["kdcode"].isin(t["holdings"])][
                    "open_to_open_return"
                ].dropna()
                if len(t_returns) > 0:
        ...
        for tid, t in tranches.items():
            ...
            realized = t_data[["kdcode", "open_to_open_return"]].dropna(
                subset=["open_to_open_return"]
            )
            if len(realized) == 0:
                continue
                ...
                daily_holdings_records.append(
                    {
                        ...
                        "kdcode": held_kdcode,
                        "tranche_id": tid,
                        "rank": int(current_ranks.get(held_kdcode, 0))
                        if held_kdcode in current_ranks
                        else np.nan,
                        "score": float(day_score_map.get(held_kdcode, np.nan)),
            ...
            trade_records.append(
                {
                    ...
                    "rank": int(current_ranks.get(bought_kdcode, 0))
                    if bought_kdcode in current_ranks
                    else np.nan,
                    "score": float(day_score_map.get(bought_kdcode, np.nan)),
                }
            ...
            trade_records.append(
                {
                    ...
                    "action": "SELL",
                    "tranche_id": tranche_id,
                    "rank": int(current_ranks.get(sold_kdcode, 0))
                    if sold_kdcode in current_ranks
                    else np.nan,
                    "score": float(day_score_map.get(sold_kdcode, np.nan)),
                }
...
def simulate_trading_strategy_block(
    predictions_df,
    stock_data_df,
    top_k=10,
    label_t=5,
    ...
    print(
        f"  Block rebalance: entire portfolio of {top_k} stocks rebalanced every {holding_period} days (retail-style)"
    )
    print("  Portfolio: open-to-open returns (entry at T+1 open, exit at T+2 open)")
    print("  Benchmark: open-to-open returns (equal-weighted, same window as portfolio)")
    print(f"  Transaction costs: {cost_status}")
    ...
    for i, pred_date in enumerate(pred_dates):
        ...
        if len(day_preds) < top_k:
            ...
            held_returns = entry_day_data[entry_day_data["kdcode"].isin(current_holdings)][
                "open_to_open_return"
            ].dropna()
            ...
            # daily_holdings
            day_score_map = (
                day_preds.set_index("kdcode")["score"].to_dict() if len(day_preds) > 0 else {}
            )
                    ...
                    daily_holdings_records.append(
                        {
                            ...
                            "tranche_id": 0,
                            "rank": np.nan,
                            "score": day_score_map.get(kd, np.nan),
                            "weight": w,
                            "stock_return": r,
                            "contribution": r * w,
                            "rank_gate_enabled": gate_enabled,
        ...
        try:
            entry_date_idx = all_stock_dates.index(pred_date) + 1
            entry_date_this = (
                all_stock_dates[entry_date_idx] if entry_date_idx < len(all_stock_dates) else None
            )
        except (ValueError, IndexError):
            entry_date_this = None

        if is_rebalance_day:
            if not gate_enabled or not current_holdings or prev_ranks is None:
                top_stocks = day_preds.head(top_k)["kdcode"].tolist()
            else:
                ...
                trade_records.append(
                    {
                        ...
                        "action": "BUY",
                        "tranche_id": 0,
                        "rank": int(current_ranks.get(bought_kd, 0))
                        if bought_kd in current_ranks
                        else np.nan,
                ...
                trade_records.append(
                    {
                        ...
                        "action": "SELL",
                        "tranche_id": 0,
                        "rank": int(current_ranks.get(sold_kd, 0))
                        if sold_kd in current_ranks
                        else np.nan,
                ...
                daily_holdings_records.append(
                    {
                        ...
                        "kdcode": kd,
                        "tranche_id": 0,
                        "rank": int(current_ranks.get(kd, 0)) if kd in current_ranks else np.nan,
                        "score": float(day_score_map.get(kd, np.nan)),
                        "weight": w,
                        "stock_return": r,
                        "contribution": r * w,
                        "rank_gate_enabled": gate_enabled,
...
def evaluate(predictions_dir, config=None):
    ...
    stock_data = load_stock_data(
        config["data_file"], start_date=config["test_start"], end_date=config["test_end"]
    )
...
def print_results(results, model_name="MCI-GRU", num_tests=1, adjustment_method="bhy"):
    ...
    if num_tests > 1:
        ...
        haircut_result = haircut_sharpe_ratio(
            results["ASR"], num_years, num_tests, method=adjustment_method, rank=1
        )
    ...
    print("  " + "-" * 66)
    print(
        f"  {'Total Portfolio Return':<25} {results['total_return']:>12.4f}  ({results['total_return'] * 100:.2f}%)"
    )
    print(
    ...
    if results.get("transaction_costs_enabled", False):
        ...
        print("  " + "-" * 66)
        print(
            f"  {'Bid-Ask Spread':<25} {results['tc_bid_ask_spread_bps']:>12.1f} bps (round-trip)"
        )
        print(f"  {'Slippage':<25} {results['tc_slippage_bps']:>12.1f} bps (per trade)")
        print("  " + "-" * 66)
        print(
            f"  {'Total Transaction Cost':<25} {results['total_transaction_cost'] * 100:>11.4f}%  (cumulative)"
        )
        ...
        print("  " + "-" * 66)
        print(
            f"  {'Gross ARR (before costs)':<25} {results['gross_ARR']:>12.4f}  ({results['gross_ARR'] * 100:.2f}%)"
        )
        print(
            f"  {'Net ARR (after costs)':<25} {results['net_ARR']:>12.4f}  ({results['net_ARR'] * 100:.2f}%)"
        )
        print(
            f"  {'Cost Drag on ARR':<25} {results['cost_drag_ARR']:>12.4f}  ({results['cost_drag_ARR'] * 100:.2f}%)"
        )
        print(
            f"  {'Gross Total Return':<25} {results['gross_total_return']:>12.4f}  ({results['gross_total_return'] * 100:.2f}%)"
        )
        print(
            f"  {'Net Total Return':<25} {results['net_total_return']:>12.4f}  ({results['net_total_return'] * 100:.2f}%)"
        )
        print("  " + "-" * 66)
        print()
    else:
...
def derive_portfolio_composition(
    holdings_df: pd.DataFrame, trades_df: pd.DataFrame
) -> pd.DataFrame:
    """
...
def derive_holdings_summary(holdings_df: pd.DataFrame) -> pd.DataFrame:
    ...
    win_rate_df = (
        holdings_df.groupby("kdcode", as_index=False)["stock_return"]
    ...
    summary_df = summary_df.sort_values("total_contribution", ascending=False).reset_index(
        drop=True
    )
...
def save_backtest_results(
    ...
    if sim_results:
        ...
        returns_df = pd.DataFrame(
            {
                ...
                "excess_return": sim_results["portfolio_returns"]
                - sim_results["benchmark_returns"],
            }
        )

        if sim_results.get("transaction_costs_enabled", False):
            returns_df["gross_portfolio_return"] = sim_results.get(
                "gross_portfolio_returns", sim_results["portfolio_returns"]
            )
            returns_df["transaction_cost"] = (
                returns_df["gross_portfolio_return"] - returns_df["portfolio_return"]
            )
        ...
        if len(holdings_df) > 0:
            ...
            day_contribution_sum = attribution_df.groupby("entry_date")["contribution"].transform(
                "sum"
            )
    ...
    with open(summary_file, "w") as f:
        ...
        if config:
            f.write(
                f"Test Period: {config.get('test_start', 'N/A')} to {config.get('test_end', 'N/A')}\n"
            )
            ...
            if config.get("transaction_costs", {}).get("enabled", False):
                ...
                f.write("Transaction Costs: Enabled\n")
                f.write(f"  Bid-Ask Spread: {tc.get('bid_ask_spread', 0) * 10000:.1f} bps\n")
                f.write(f"  Slippage: {tc.get('slippage', 0) * 10000:.1f} bps\n")
            else:
                f.write("Transaction Costs: Disabled\n")
            if results.get("rank_gate_enabled", False):
                f.write(
                    f"Rank-Drop Gate: Enabled (min_rank_drop={results.get('min_rank_drop', 'N/A')})\n"
                )
                f.write(
                    f"  Days with gate-triggered exits: {results.get('days_with_gate_exits', 0)}\n"
                )
                f.write(
                    f"  Days skipped (empty post-gate portfolio): {results.get('days_skipped_by_rank_gate', 0)}\n"
                )
            else:
                f.write("Rank-Drop Gate: Disabled\n")
            f.write("\n")

        f.write("Key Metrics:\n")
        f.write("-" * 80 + "\n")
        f.write(
            f"  ARR (Annualized Return):       {results.get('ARR', 0):.4f} ({results.get('ARR', 0) * 100:.2f}%)\n"
        )
        f.write(
            f"  AVoL (Annualized Volatility):  {results.get('AVoL', 0):.4f} ({results.get('AVoL', 0) * 100:.2f}%)\n"
        )
        f.write(
            f"  MDD (Maximum Drawdown):        {results.get('MDD', 0):.4f} ({results.get('MDD', 0) * 100:.2f}%)\n"
        )
        ...
        if num_tests > 1:
            ...
            f.write(
                f"  Statistical Significance: {'Yes' if haircut_result['is_significant'] else 'No'} (p={haircut_result['adjusted_p_value']:.4f})\n"
            )
            f.write("\n")
...
def setup_backtest_tracking(
    ...
    if linked_metadata is not None:
        ...
        resolved_experiment_name = resolved_experiment_name or _infer_experiment_name(
            predictions_dir
        )
...
def evaluate_multiple_models(save_path, num_models=10, config=None):
    ...
    for model_id in range(num_models):
        ...
        try:
            ...
            if results:
                ...
                print(
                    f"    ARR={results['ARR']:.4f}, ASR={results['ASR']:.4f}, IR={results['IR']:.4f}"
                )
    ...
    return {"individual": all_results, "averaged": avg_results}
...
def main():
    ...
    if args.multi_model:
        ...
        multi_results = evaluate_multiple_models(
            args.multi_model, num_models=args.num_models, config=config
        )
        ...
        if results:
            ...
            if args.auto_save and backtest_dir:
                ...
                stock_data = load_stock_data(
                    config["data_file"], config["test_start"], config["test_end"]
                )
                ...
                if args.plot or args.auto_save:
                    ...
                    plot_equity_curve(
                        args.predictions_dir, stock_data, config, output_path=plot_file
                    )
                ...
                if args.plot:
                    ...
                    plot_output = (
                        args.output.replace(".csv", "_equity.png") if args.output else None
                    )
