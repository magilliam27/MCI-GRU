...
Sibling of ``test_gics_top10_110_config.py``. The two configs share a selector
but not their measured constants: the 2021-start universe has a measured floor of
109 admissible names per session, the 2016-start one has 108, so each needs its
own threshold assertion rather than a shared one.

The same two properties fail invisibly rather than loudly:
...
CONFIG_PATH = REPO_ROOT / "configs" / "data" / "gics_top10_110_2016.yaml"

# configs/config.yaml model.label_t
LABEL_T = 5

# Measured minimum admissible names per session across the 2658 sessions from
# 2016-01-04 to 2026-07-30 on the pulled panel.
MEASURED_MIN_ADMISSIBLE = 108
...
def test_pit_filtering_is_enabled_because_it_defines_the_universe():
    ...
    assert cfg["use_pit_universe"] is True, (
        "Without PIT filtering this panel resolves to all 206 names that ever "
...
def test_train_start_opens_with_the_universe_not_with_the_price_panel():
    cfg = _config()

    assert cfg["train_start"] == "2016-01-04", (
        "train_start must be the universe opening date. The price panel starts "
        "2015-01-02 to give the correlation graph 252 sessions of pre-history; "
        "training on that pre-history would score names before the PIT universe "
        "admits them."
    )
