...
def run_backtest(
    predictions_dir: str, experiment_config: dict, log_file: str | None = None
) -> tuple[bool, float]:
    """
    ...
    for i, exp_name in enumerate(experiments, 1):
        ...
        if not backtest_only:
            ...
            if not train_success:
                log_message("  Skipping backtest due to training failure", log_file)
        ...
        if not train_only:
            ...
            backtest_success, backtest_time = run_backtest(
                predictions_dir=predictions_dir, experiment_config=exp_config, log_file=log_file
            )
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Run temporal robustness experiments for MCI-GRU")
    ...
    parser.add_argument(
        "--num_epochs", type=int, default=100, help="Number of training epochs (default: 100)"
    )
    ...
    parser.add_argument(
        "--train_only", action="store_true", help="Only run training, skip backtests"
    )
...
def main():
    ...
    all_success = all(
        (
            result.get("training", {}).get("success", True)
            and result.get("backtest", {}).get("success", True)
        )
