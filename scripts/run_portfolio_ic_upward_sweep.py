"""Portfolio-IC Upward Weight Sweep foreground, resumable Colab runner."""
...
RUNNER_NAME = "run_portfolio_ic_upward_sweep.py"
NOTEBOOK_CONTROL_FLAGS = ("RESUME_RUN_ROOT", "AUTO_UNASSIGN_ON_FINISH")
...
def detect_gpu_name() -> str:
    ...
    upper = name.upper()
    if any(marker in upper for marker in BLOCKED_GPU_NAMES):
        raise RuntimeError(
            f"Refusing runtime GPU {name}. Expected G4/L4-class Colab runtime, not T4/CPU."
        )
    ...
    if STRICT_GPU_MARKERS and not any(marker in upper for marker in STRICT_GPU_MARKERS):
        raise RuntimeError(f"GPU {name} does not match STRICT_GPU_MARKERS={STRICT_GPU_MARKERS}.")
    return name
...
def start_gpu_sampler(repo_dir: Path, summary_dir: Path) -> tuple[subprocess.Popen[str], Path] | tuple[None, None]:
    ...
    gpu_util_path = summary_dir / "gpu_util.csv"
    stop_path = summary_dir / "gpu_util.stop"
    if stop_path.exists():
        stop_path.unlink()
...
def stop_gpu_sampler(proc: subprocess.Popen[str] | None, stop_path: Path | None) -> None:
    if proc is None or stop_path is None:
        return
...
def main(argv: list[str] | None = None) -> int:
    ...
    gpu_sampler_proc, gpu_sampler_stop_path = start_gpu_sampler(repo_dir, summary_dir)
    try:
        ...
        stop_gpu_sampler(gpu_sampler_proc, gpu_sampler_stop_path)
