...
def find_latest_run(base_dir: str, experiment_name: str) -> str | None:
...
def load_backtest_results(backtest_dir: str) -> dict | None:
...
def load_daily_returns(backtest_dir: str) -> pd.DataFrame | None:
...
def load_all_experiment_results(results_dir: str) -> dict[str, dict]:
    ...
    for exp_name, exp_meta in TEMPORAL_EXPERIMENTS.items():
        ...
        if metrics:
            ...
            print(
                f"  Loaded {exp_name}: ARR={metrics.get('ARR', 0):.4f}, ASR={metrics.get('ASR', 0):.4f}"
            )
        else:
    ...
    for col in key_cols:
        if col in comparison_df.columns:
            ...
            metrics[col] = {
                ...
                "cv": float(np.std(values) / np.abs(np.mean(values)))
                if np.mean(values) != 0
                else 0,
...
def generate_comparison_plot(comparison_df: pd.DataFrame, output_path: str):
    ...
    plt.suptitle(
        "Temporal Robustness Analysis: MCI-GRU Performance Across Test Periods",
        fontsize=14,
        fontweight="bold",
        y=1.02,
    )
...
def generate_equity_curves_plot(results: dict[str, dict], output_path: str):
    ...
    for i, (_exp_name, data) in enumerate(sorted(results.items())):
...
def generate_consistency_report(
    comparison_df: pd.DataFrame, consistency_metrics: dict, output_path: str
):
    ...
    with open(output_path, "w") as f:
        ...
        f.write(
            f"The MCI-GRU model was evaluated across **{len(comparison_df)} independent test periods** "
        )
        f.write(
            f"({', '.join(comparison_df['Test Period'].values)}) to assess temporal robustness.\n\n"
        )
        ...
        f.write(
            f"(ARR CV: {consistency.get('arr_cv', 0) * 100:.1f}%, ASR CV: {consistency.get('asr_cv', 0) * 100:.1f}%)\n\n"
        )
        ...
        for metric_name in ["ARR", "ASR", "MDD", "IR", "CR"]:
            if metric_name in consistency_metrics:
                ...
                f.write(
                    f"{m['min'] * scale:.2f} | {m['max'] * scale:.2f} | {m['cv'] * 100:.1f}% |\n"
                )
        ...
        if is_consistent:
            f.write(
                "The model demonstrates **temporal consistency**, with coefficient of variation "
            )
        ...
        f.write("---\n")
        f.write("*Report generated by analyze_temporal_consistency.py*\n")

    print(f"  Saved: {output_path}")


def generate_summary_text(comparison_df: pd.DataFrame, consistency_metrics: dict, output_path: str):
...
def run_analysis(results_dir: str, output_dir: str) -> bool:
    """
    ...
    consistency = consistency_metrics.get("overall_consistency", {})
    print(
        f"  Consistency: {'CONSISTENT' if consistency.get('is_consistent', False) else 'VARIABLE'}"
    )
...
def main():
    ...
    success = run_analysis(results_dir=args.results_dir, output_dir=output_dir)
