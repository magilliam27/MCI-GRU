...
def compare_predictions(new_dir: str, old_dir: str, tolerance: float = 1e-4) -> dict:
    ...
    for filename in sorted(common_files):
        ...
        merged = new_df.merge(old_df, on=["kdcode", "dt"], suffixes=("_new", "_old"))
        ...
        if max_diff < tolerance:
            ...
            results["mismatched_files"].append(
                {"file": filename, "max_diff": max_diff, "avg_diff": avg_diff}
            )
...
def verify_graph_builder():
    ...
    try:
        ...
        edge_index, edge_weight = gb.build_graph(df, stocks, "2020-12-01", show_progress=False)
...
def verify_config():
    ...
    try:
        ...
        # Test validation
        try:
            DataConfig(train_start="2025-01-01", train_end="2020-01-01")
